...
<head>
    ...
    <style>
        /* ===== Variables thème (identiques à postuler/index) ===== */
        :root{
        ...
        *{box-sizing:border-box}
        ...
        /* ===== Scroll progress (comme postuler.php) ===== */
        #scrollProgress{
        ...
        nav.navbar{
            ...
            gap:20px; padding:12px 16px;background:rgba(11,19,38,1);
            border-bottom:1px solid var(--border);
            transition:background-color .35s, box-shadow .35s !important;
        }
        nav.navbar.nav--scrolled{background:rgba(10,15,31,.92);box-shadow:0 10px 24px rgba(0,0,0,.25);backdrop-filter:saturate(120%) blur(6px)}
        .logo img{width:280px;max-width:48vw}
        .nav-links{display:flex;gap:1.2rem;list-style:none;margin:0;padding:0}
        .nav-links a{color:var(--ink);opacity:.92;font-weight:700;position:relative}
        .nav-links a:hover,.nav-links a.active{opacity:1}
        .nav-links a::after{
            content:"";position:absolute;left:50%;bottom:-6px;width:0;height:2px;background:linear-gradient(90deg,var(--pi-blue),var(--pi-red));
            transition:width .25s,left .25s;
        }
        .nav-links a:hover::after,.nav-links a.active::after{width:100%;left:0}
        .nav-buttons a{display:flex;flex-direction:column;align-items:center;gap:2px;color:var(--ink);font-weight:700}
        ...
        /* ===== Hero + halos (paristanbul) ===== */
        .hero{position:relative;isolation:isolate;padding:clamp(48px,6vw,72px) 0;overflow:hidden}
        ...
        .tl-card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:14px 16px;transform-style:preserve-3d;transition:transform .12s,box-shadow .2s}
        ...
        .stat{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:16px;text-align:center;transform-style:preserve-3d}
        ...
        /* ===== Footer ===== */
        .footer{background:#0A0F1F;color:var(--ink);padding:40px 18px 22px;border-top:1px solid var(--border)}
        .footer .grid{display:grid;grid-template-columns:1.2fr repeat(3,1fr) 1.2fr;gap:18px;width:min(1100px,92vw);margin:auto}
        @media(max-width:960px){.footer .grid{grid-template-columns:1fr 1fr;gap:24px}}
        @media(max-width:560px){.footer .grid{grid-template-columns:1fr}}
        .footer a{color:var(--muted)} .footer a:hover{color:#fff}
        .social{display:flex;gap:10px;margin-top:10px}
        .social a{width:44px;height:44px;border-radius:50%;display:grid;place-items:center;color:#fff}
        .social .facebook{background:#1877F2}.social .instagram{background:radial-gradient(circle at 30% 107%,#fdf497 0%,#fd5949 50%,#d6249f 75%,#285AEB 100%)}.social .tiktok{background:#000}.social .youtube{background:#f00}.social .paristanbul{background:#fff;color:#000}
        .newsletter-form{display:flex;background:#151e31;border-radius:8px;overflow:hidden;margin-top:10px}
        .newsletter-form input{flex:1;padding:.75rem .9rem;border:0;background:transparent;color:var(--ink)}
        .newsletter-form button{border:0;background:var(--pi-blue);color:#fff;padding:0 16px;font-size:1.2rem}

        /* ===== Reveal / Ripple / ToTop / Loader ===== */
        @media(prefers-reduced-motion:reduce){.reveal,.media,nav.navbar,.btn,.tilt{transition:none!important;animation:none!important}}
        ...
        #piLoader{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;background:
                ...
                radial-gradient(1000px 700px at 85% 15%,rgba(214,69,46,.16),transparent 55%),#0A0F1F;transition:opacity .4s,visibility .4s}
...
<nav class="navbar">
    <div class="logo">
        <a href="index.php"><img src="../assets/img/paristanbul_logo_1200x350-1024x299.png" alt="Paristanbul"></a>
    </div>
    <ul class="nav-links">
        <li><a href="index.php">Accueil</a></li>
        <li><a href="nosMagasins.php">Nos magasins</a></li>
        <li><a href="quiSommesNous.html" class="active">Notre histoire</a></li>
        <li><a href="postuler.php">Postuler</a></li>
    </ul>
    <div class="nav-buttons">
        <a href="pageConnexion.php" class="magnet">
            <i class="bi bi-person" style="font-size:28px"></i>
            <span style="font-size:.9rem">Me connecter / M'inscrire</span>
        </a>
    </div>
</nav>
    ...
    <div class="container story-grid">
        ...
        <div class="reveal" style="--reveal-delay:80ms" id="heroText">
            ...
            <h1 style="margin:.35rem 0 .6rem;line-height:1.06;letter-spacing:-.02em;font-weight:900">
                Une histoire de famille, de courage et de goût.
            </h1>
...
<footer class="footer">
    <div class="grid">
        <div>
            ...
            <div class="social">
                <a class="facebook" href="https://www.facebook.com/supermarcheparistanbul/"><i class="bi bi-facebook"></i></a>
                <a class="instagram" href="https://www.instagram.com/paristanbul_supermarche/?hl=fr"><i class="bi bi-instagram"></i></a>
                <a class="tiktok" href="https://www.tiktok.com/@supermarche_paristanbul"><i class="bi bi-tiktok"></i></a>
                <a class="youtube" href="https://www.youtube.com/channel/UCsjy3bdpFzBwM7MF923gKvA"><i class="bi bi-youtube"></i></a>
                <a class="paristanbul" href="#" title="Application"><img src="../assets/img/logo.app.pi.webp" alt="PI" style="width:70%;height:70%;object-fit:contain"></a>
            </div>
        </div>
        <div>
            <h3>L’enseigne</h3>
            <ul>
                ...
                <li><a href="pageConnexion.php#contact">Nous contacter</a></li>
            </ul>
        </div>
        <div>
            <h3>Actualités</h3>
        ...
        <div>
            ...
            <ul>
                ...
                <li><a href="postuler.php#candidature">Candidature spontanée</a></li>
            </ul>
        </div>
        <div>
            <h3>Newsletters</h3>
            <p>Abonnez-vous pour recevoir nos dernières actualités.</p>
            <form class="newsletter-form" action="#" method="post" onsubmit="event.preventDefault();alert('Merci ! Vous êtes inscrit(e).');">
                <input type="email" placeholder="Votre email" required>
                <button type="submit" title="S’inscrire"><i class="bi bi-arrow-right"></i></button>
            </form>
        </div>
    </div>
</footer>
...
<!-- ===== JS (mêmes interactions que postuler/index) ===== -->
<script>
    /* Scroll progress */
    (function(){
    ...
    /* Loader robuste */
    (function(){
        const loader=document.getElementById('piLoader'); if(!loader) return;
        let done=false; const hide=()=>{if(done)return;done=true;loader.classList.add('hide');setTimeout(()=>loader.remove(),800);};
        const start=Date.now(),MIN=500; const afterMin=fn=>setTimeout(fn,Math.max(0,MIN-(Date.now()-start)));
        if(document.readyState==='complete'){afterMin(hide);return;}
        window.addEventListener('DOMContentLoaded',()=>afterMin(hide),{once:true});
        window.addEventListener('load',()=>afterMin(hide),{once:true});
        setTimeout(hide,4000);
    })();

    /* Navbar + ToTop */
    (function(){
        const nav=document.querySelector('nav.navbar'); const toTop=document.getElementById('toTop');
        const onS=()=>{ const y=window.scrollY||0; nav.classList.toggle('nav--scrolled', y>8); toTop.classList.toggle('show', y>500); };
    ...
    (function(){
        ...
        (function raf(){
            ...
            if(media) media.style.transform=`translate3d(${cx*-10}px,${cy*-10}px,0)`;
    ...
    /* Lien store (redirection intelligente) */
